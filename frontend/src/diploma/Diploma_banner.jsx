import React from "react";
import { Link } from "react-router-dom";

const Diploma_banner = () => {
  return (
    <>
      <div className="max-w-screen-2xl rounded-lg">
        <section data-aos="zoom-out" className="overflow-hidden md:h-screen  bg-[url(/IMG_DIR/GamvirFull1.jpg)] bg-cover bg-top bg-no-repeat">
          <div className="bg-black/50  p-8  md:h-screen md:p-12 lg:px-16 lg:py-24">
            <div className="text-left ">
              <h2 data-aos="fade-down" data-aos-duration="900" className="text-2xl font-bold mt-10 text-white sm:text-3xl md:text-5xl">
                Engineering at Gamvir
              </h2>

              <p data-aos="zoom-in-down" data-aos-duration="1000" className=" max-w-lg text-white/90 mt-8 md:mt-6 md:block md:text-lg md:leading-relaxed">
                Gamvir Samudra Setu, is one of the well known technical schools established in 2073 BS under Technical Education in Community School (TECS) affiliated to Council for Technical Education & Vocational Training (CTEVT) which is leading
                technical education board in Nepal.
              </p>

              <div data-aos="fade-up" data-aos-duration="1000" className="mt-4 sm:mt-8 hidden md:block">
                <Link to={"/apply"}>
                  <span className=" rounded-full bg-emerald-600 px-12 py-3 text-sm font-medium text-white transition hover:bg-emerald-700 focus:outline-none focus:ring focus:ring-yellow-400">Apply Now</span>
                </Link>
              </div>
            </div>
          </div>
        </section>
      </div>
    </>
  );
};

export default Diploma_banner;
